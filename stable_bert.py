...
def predict_and_save_predictions(model, dataloader, device="cuda"):
    model.eval()
    model.to(device)
    ...
    # --- Failure Prediction / Calibration Metrics Helper ---
    def compute_failure_metrics(y_true_correct, y_confidence, prefix):
        """
        Calculates and logs failure prediction metrics.
        
        Args:
            y_true_correct (np.array): Boolean array where True indicates correct prediction.
            y_confidence (np.array): Confidence scores (higher means more confident).
            prefix (str): Prefix for logging metrics (e.g., 'conf' or 'mcp').
        """
        # Targets
        y_true_error = (~y_true_correct).astype(int)
        y_true_success = y_true_correct.astype(int)
        
        # Uncertainty score (higher means more likely to be an error)
        # We assume confidence is in [0, 1]. 
        y_score_error = 1.0 - y_confidence

        # 1. AUC (Area Under ROC) - Separation between Correct and Error
        auroc = roc_auc_score(y_true_error, y_score_error)

        # 2. AUPR-Error (Precision-Recall for Errors)
        # Target: Error (1). Score: Uncertainty
        precision_err, recall_err, _ = precision_recall_curve(y_true_error, y_score_error)
        aupr_error = auc(recall_err, precision_err)

        # 3. AUPR-Success (Precision-Recall for Success)
        # Target: Success (1). Score: Confidence
        ...
        # 4. FPR at 95% TPR
        # We want TPR (Recall of Errors) to be 0.95. What is the FPR (False Alarm Rate)?
    ...
    # Determine Correct/Incorrect predictions (Boolean mask)
    is_correct = (y_true_int == y_pred_int)

    # 1. Metrics for Learned Confidence (ConfidNet)
    compute_failure_metrics(is_correct, y_conf, prefix="conf")

    # 2. Metrics for Baseline MCP (Max Class Probability)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        file_path = os.path.join(tmpdir, "predictions.parquet")
...
predict_and_save_predictions(model, test_dataloader, test_dataset)
