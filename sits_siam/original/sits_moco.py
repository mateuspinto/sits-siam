...
class OriginalSITSMoco(nn.Module):
    def __init__(self, num_classes=13, pretrained=True):
        super(OriginalSITSMoco, self).__init__()
        self.model = STNet(num_classes=num_classes)

        if pretrained:
            model_dict = self.model.state_dict()
            pretrain_model = torch.load(
                "weights/original_sits_moco/model_best.pth",
            ...
            for k in list(pretrain_model.keys()):
                if (
                    ...
                    state_dict[k[len("encoder_q.") :]] = pretrain_model[k]
    ...
    def forward(self, sample):
        ...
        return self.model((x, mask, doy, weight))
